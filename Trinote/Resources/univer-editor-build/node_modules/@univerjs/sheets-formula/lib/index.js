...
let TriggerCalculationController = class TriggerCalculationController extends Disposable {
	...
	_generateDirty(commands) {
		...
		for (const command of commands) {
			...
			const { dirtyRanges, dirtyNameMap, dirtyDefinedNameMap, dirtyUnitFeatureMap, dirtyUnitOtherFormulaMap, clearDependencyTreeCache, forceCalculation = false } = conversion.getDirtyData(command);
			if (dirtyRanges != null) this._mergeDirtyRanges(allDirtyRanges, dirtyRanges);
			if (dirtyNameMap != null) this._mergeDirtyNameMap(allDirtyNameMap, dirtyNameMap);
			if (dirtyDefinedNameMap != null) this._mergeDirtyNameMap(allDirtyDefinedNameMap, dirtyDefinedNameMap);
			...
			if (clearDependencyTreeCache != null) this._mergeDirtyNameMap(allClearDependencyTreeCache, clearDependencyTreeCache);
	...
	*/
	...
	_mergeDirtyNameMap(allDirtyNameMap, dirtyNameMap) {
		Object.keys(dirtyNameMap).forEach((unitId) => {
			if (allDirtyNameMap[unitId] == null) allDirtyNameMap[unitId] = {};
			Object.keys(dirtyNameMap[unitId]).forEach((sheetId) => {
				var _dirtyNameMap$unitId;
				if ((_dirtyNameMap$unitId = dirtyNameMap[unitId]) === null || _dirtyNameMap$unitId === void 0 ? void 0 : _dirtyNameMap$unitId[sheetId]) allDirtyNameMap[unitId][sheetId] = dirtyNameMap[unitId][sheetId];
			});
		});
	}
...
function getFormulaReferenceMoveUndoRedo(oldFormulaData, newFormulaData, formulaReferenceMoveParam) {
	...
	if (formulaReferenceSheetList.includes(type)) return getFormulaReferenceSheet(oldFormulaData, newFormulaData);
...
//#endregion
export { CalculationMode, DescriptionService, FormulaAutoFillController, FormulaRefRangeService, IDescriptionService, IRegisterFunctionService, IRemoteRegisterFunctionService, ImageFormulaCellInterceptorController, InsertFunctionCommand, PLUGIN_CONFIG_KEY_BASE, QuickSumCommand, RegisterFunctionService, RemoteRegisterFunctionService, TriggerCalculationController, UniverRemoteSheetsFormulaPlugin, UniverSheetsFormulaPlugin, UpdateDefinedNameController, UpdateFormulaController, calculateFormula };