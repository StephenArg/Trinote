...
function addCustomDecorationBySelectionFactory(accessor, param) {
	...
	const documentDataModel = propUnitId ? univerInstanceService.getUnit(propUnitId, _univerjs_core.UniverInstanceType.UNIVER_DOC) : univerInstanceService.getCurrentUnitForType(_univerjs_core.UniverInstanceType.UNIVER_DOC);
...
let DocAutoFormatService = class DocAutoFormatService extends _univerjs_core.Disposable {
	...
	onAutoFormat(id, params) {
		...
		const unit = this._univerInstanceService.getCurrentUnitForType(_univerjs_core.UniverInstanceType.UNIVER_DOC);
...
let DocMenuStyleService = class DocMenuStyleService extends _univerjs_core.Disposable {
	...
	getDefaultStyle() {
		...
		const docDataModel = this._univerInstanceService.getCurrentUnitForType(_univerjs_core.UniverInstanceType.UNIVER_DOC);
...
const BreakLineCommand = {
	...
	handler: (accessor, params) => {
		...
		const docDataModel = univerInstanceService.getCurrentUnitForType(_univerjs_core.UniverInstanceType.UNIVER_DOC);
...
const DeleteLeftCommand = {
	...
	handler: async (accessor) => {
		...
		if ((0, _univerjs_engine_render.isFirstGlyph)(curGlyph) && preGlyph !== curGlyph && (isBullet === true || isIndent === true) && collapsed) {
...
var HtmlToUDMService = class HtmlToUDMService {
	...
	_process(parent, nodes, doc) {
		...
		for (const node of nodes) if (node.nodeType === Node.TEXT_NODE) {
			var _node$nodeValue, _node$nodeValue2;
			if (((_node$nodeValue = node.nodeValue) === null || _node$nodeValue === void 0 ? void 0 : _node$nodeValue.trim()) === "") continue;
			const text = (_node$nodeValue2 = node.nodeValue) === null || _node$nodeValue2 === void 0 ? void 0 : _node$nodeValue2.replace(/[\r\n]/g, "");
			let style;
		...
		switch (tagName) {
			case "TABLE": {
				if (body.dataStream[body.dataStream.length - 1] !== "\r") {
					...
					(_body$paragraphs = body.paragraphs) === null || _body$paragraphs === void 0 || _body$paragraphs.push({ startIndex: body.dataStream.length - 1 });
				}
		...
		switch (tagName) {
			...
			case "TD":
				...
				if (body.dataStream[body.dataStream.length - 1] !== "\r") {
					...
					(_body$paragraphs2 = body.paragraphs) === null || _body$paragraphs2 === void 0 || _body$paragraphs2.push({ startIndex: body.dataStream.length });
					body.dataStream += "\r";
				}
				(_body$sectionBreaks = body.sectionBreaks) === null || _body$sectionBreaks === void 0 || _body$sectionBreaks.push({ startIndex: body.dataStream.length });
				body.dataStream += `\n${_univerjs_core.DataStreamTreeTokenType.TABLE_CELL_END}`;
				break;
		}
	}
...
function covertTextRunToHtml(dataStream, textRun) {
	...
	if (ff) style.push(`font-family: ${ff}`);
	if (cl) style.push(`color: ${cl.rgb}`);
	if (fs) style.push(`font-size: ${fs}pt`);
	if (ol) style.push("text-decoration: overline");
	if (bg) style.push(`background: ${bg.rgb}`);
...
function getBodyInlineSlice(body, startIndex, endIndex) {
	...
	if (cursorIndex !== endIndex) spanList.push(dataStream.slice(cursorIndex, endIndex));
...
function processNode(node, doc, result) {
	switch (node.nodeType) {
		...
		case _univerjs_core.DataStreamTreeNodeType.PARAGRAPH: {
			...
			const { paragraphStyle = {} } = (_doc$body$paragraphs$ = (_doc$body = doc.body) === null || _doc$body === void 0 || (_doc$body = _doc$body.paragraphs) === null || _doc$body === void 0 ? void 0 : _doc$body.find((p) => p.startIndex === endIndex)) !== null && _doc$body$paragraphs$ !== void 0 ? _doc$body$paragraphs$ : {};
			...
			if (children.length) for (const table of children) processNode(table, doc, result);
...
let DocClipboardService = class DocClipboardService extends _univerjs_core.Disposable {
	...
	async legacyPaste(options) {
		...
		const currentDocInstance = this._univerInstanceService.getCurrentUnitForType(_univerjs_core.UniverInstanceType.UNIVER_DOC);
	...
	async _paste(docData) {
		...
		if (!((_this$_univerInstance = this._univerInstanceService.getCurrentUnitForType(_univerjs_core.UniverInstanceType.UNIVER_DOC)) === null || _this$_univerInstance === void 0 ? void 0 : _this$_univerInstance.getUnitId())) return false;
	...
	async _setClipboardData(documentList, needCache = true) {
		...
		if (documentList.length === 1 && needCache) {
			...
			if ((_doc$body2 = doc.body) === null || _doc$body2 === void 0 || (_doc$body2 = _doc$body2.customBlocks) === null || _doc$body2 === void 0 ? void 0 : _doc$body2.length) {
				cache.drawings = {};
	...
	_getDocumentBodyInRanges(sliceType, ranges) {
		...
		for (const range of allRanges) {
			...
			if (rangeType === _univerjs_core.DOC_RANGE_TYPE.RECT) {
				...
				let bodySlice;
				if (!spanEntireRow) bodySlice = getTableSlice(body, startOffset, endOffset);
				else bodySlice = getTableSlice(body, startOffset, endOffset);
	...
	async _createImagePasteHtml(files) {
		var _this$_clipboardHooks, _this$_clipboardHooks2;
		const doc = {
			id: "",
			documentStyle: {},
			body: {
		...
		const onBeforePasteImage = (_this$_clipboardHooks = (_this$_clipboardHooks2 = this._clipboardHooks.find((e) => e.onBeforePasteImage)) === null || _this$_clipboardHooks2 === void 0 ? void 0 : _this$_clipboardHooks2.onBeforePasteImage) !== null && _this$_clipboardHooks !== void 0 ? _this$_clipboardHooks : fileToBase64;
...
const DocSelectAllCommand = {
	...
	handler: async (accessor) => {
		...
		for (const table of tables) {
			const { startIndex, endIndex } = table;
			if (offset !== startIndex) textRanges.push({
				startOffset: offset,
		...
		if (offset !== body.dataStream.length - 2) textRanges.push({
			startOffset: offset,
...
const OrderListCommand = {
	...
	handler: (accessor, params) => {
		const commandService = accessor.get(_univerjs_core.ICommandService);
		if (params === null || params === void 0 ? void 0 : params.value) return commandService.syncExecuteCommand(ChangeListTypeCommand.id, { listType: params.value });
		return commandService.syncExecuteCommand(ListOperationCommand.id, { listType: _univerjs_core.PresetListType.ORDER_LIST });
	}
};
...
function insertList(accessor, listType) {
	var _paragraph$bullet2, _paragraph$bullet3;
	const commandService = accessor.get(_univerjs_core.ICommandService);
	const paragraph = getCurrentParagraph(accessor);
	if (!paragraph) return false;
	...
	const textX = _univerjs_core.BuildTextUtils.selection.replace({
		...
		body: {
			...
			paragraphs: [{
				...
				bullet: {
					listType,
					listId: ((_paragraph$bullet2 = paragraph.bullet) === null || _paragraph$bullet2 === void 0 ? void 0 : _paragraph$bullet2.listType) === listType ? paragraph.bullet.listId : (0, _univerjs_core.generateRandomId)(6),
					nestingLevel: ((_paragraph$bullet3 = paragraph.bullet) === null || _paragraph$bullet3 === void 0 ? void 0 : _paragraph$bullet3.listType) === listType ? paragraph.bullet.nestingLevel : 0
				}
			}]
		}
	});
...
var NodePositionConvertToRectRange = class {
	...
	getRangePointData(startNodePosition, endNodePosition) {
		...
		const { tableId, startRowIndex: startRow, startColumnIndex: startColumn, endRowIndex: endRow, endColumnIndex: endColumn } = rectInfo;
		this._liquid.reset();
		const { pageLayoutType, pageMarginLeft, pageMarginTop } = this._documentOffsetConfig;
...
var TextRange = class {
	...
	_createOrUpdateAnchor(pointsGroup, docsLeft, docsTop, glyph) {
		...
		if (this._anchorShape) {
			...
			if (isItalic) this._anchorShape.skew(-ITALIC_DEGREE, 0);
		...
		if (isItalic) anchor.skew(-ITALIC_DEGREE, 0);
...
let DocSelectionRenderService = class DocSelectionRenderService extends _univerjs_core.RxDisposable {
	...
	*/
	...
	_removeAllCollapsedTextRanges() {
		for (const range of this._rangeList) if (range.collapsed) range.dispose();
	}
...
const CreateDocTableCommand = {
	...
	type: _univerjs_core.CommandType.COMMAND,
	handler: async (accessor, params) => {
		var _body$paragraphs, _curGlyph$parent;
...
var Editor = class extends _univerjs_core.Disposable {
	...
	*/
	focus() {
		const curDoc = this._univerInstanceService.getCurrentUnitForType(_univerjs_core.UniverInstanceType.UNIVER_DOC);
	...
	register(config, container) {
		...
		if (render) {
			...
			if (!config.scrollBar) {
				...
				(_render$mainComponent = render.mainComponent) === null || _render$mainComponent === void 0 || (_render$mainComponent = _render$mainComponent.getScene()) === null || _render$mainComponent === void 0 || (_render$mainComponent = _render$mainComponent.getViewports()) === null || _render$mainComponent === void 0 || (_render$mainComponent = _render$mainComponent[0].getScrollBar()) === null || _render$mainComponent === void 0 || _render$mainComponent.dispose();
			}
		}
		return (0, _univerjs_core.toDisposable)(() => {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: localeService.t("headerFooter.headerTopMargin") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_univerjs_design.InputNumber, {
				...
				})] }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: localeService.t("headerFooter.footerBottomMargin") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_univerjs_design.InputNumber, {
					className: "univer-mt-1.5 univer-w-4/5",
					min: 0,
					max: 200,
					precision: 1,
...
function PageSettings(props) {
	...
	const [settings, setSettings] = (0, react.useState)(() => {
		var _documentStyle$pageSi, _documentStyle$pageSi2, _documentStyle$pageSi3, _documentStyle$pageSi4, _documentStyle$pageOr, _documentStyle$margin, _documentStyle$margin2, _documentStyle$margin3, _documentStyle$margin4;
		return {
			paperSize: getPaperSize(documentStyle.pageSize),
			pageSise: {
				width: (_documentStyle$pageSi = (_documentStyle$pageSi2 = documentStyle.pageSize) === null || _documentStyle$pageSi2 === void 0 ? void 0 : _documentStyle$pageSi2.width) !== null && _documentStyle$pageSi !== void 0 ? _documentStyle$pageSi : 0,
				height: (_documentStyle$pageSi3 = (_documentStyle$pageSi4 = documentStyle.pageSize) === null || _documentStyle$pageSi4 === void 0 ? void 0 : _documentStyle$pageSi4.height) !== null && _documentStyle$pageSi3 !== void 0 ? _documentStyle$pageSi3 : 0
			},
	...
	const handlePaperSizeChange = (value) => {
		setSettings((prev) => ({
			...prev,
			paperSize: value,
			pageSise: _univerjs_core.PAGE_SIZE[value]
		}));
	};
			...
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("label", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("label", {
							className: "univer-text-sm univer-font-medium univer-text-gray-500",
...
const DocPageSetupCommand = {
	...
	handler: (accessor, params) => {
		...
		const jsonX = _univerjs_core.JSONX.getInstance();
		const { marginBottom: oldMarginBottom, marginLeft: oldMarginLeft, marginRight: oldMarginRight, marginTop: oldMarginTop, pageOrient: oldPageOrient, pageSize: oldPageSize } = docDataModel.getDocumentStyle();
		const rawActions = [];
...
const DocOpenPageSettingCommand = {
	...
	handler: (accessor) => {
		...
		const disposable = confirmService.open({
			...
			title: { label: localeService.t("page-settings.document-setting") },
			...
			onConfirm: (result) => {
				disposable.dispose();
				if (!result) return;
				const paperSize = _univerjs_core.PAGE_SIZE[result.paperSize];
		...
		activated$: new rxjs.Observable((subscriber) => {
			const disposable = commandService.onCommandExecuted((c) => {
				...
				if (id === _univerjs_docs.SetTextSelectionsOperation.id || id === AlignOperationCommand.id) {
					var _paragraph$paragraphS3;
					const paragraph = getParagraphStyleAtCursor(accessor);
					if (paragraph == null) return;
					const alignType = (_paragraph$paragraphS3 = paragraph.paragraphStyle) === null || _paragraph$paragraphS3 === void 0 ? void 0 : _paragraph$paragraphS3.horizontalAlign;
					subscriber.next(alignType === _univerjs_core.HorizontalAlign.LEFT);
				}
			});
			subscriber.next(false);
			return disposable.dispose;
	...
	return {
		...
		activated$: new rxjs.Observable((subscriber) => {
			const disposable = commandService.onCommandExecuted((c) => {
				const id = c.id;
				if (id === _univerjs_docs.SetTextSelectionsOperation.id || id === AlignOperationCommand.id) {
					var _paragraph$paragraphS4;
					const paragraph = getParagraphStyleAtCursor(accessor);
					if (paragraph == null) return;
					const alignType = (_paragraph$paragraphS4 = paragraph.paragraphStyle) === null || _paragraph$paragraphS4 === void 0 ? void 0 : _paragraph$paragraphS4.horizontalAlign;
					subscriber.next(alignType === _univerjs_core.HorizontalAlign.CENTER);
				}
			});
			subscriber.next(false);
			return disposable.dispose;
		...
		activated$: new rxjs.Observable((subscriber) => {
			const disposable = commandService.onCommandExecuted((c) => {
				...
				if (id === _univerjs_docs.SetTextSelectionsOperation.id || id === AlignOperationCommand.id) {
					...
					subscriber.next(alignType === _univerjs_core.HorizontalAlign.RIGHT);
				}
			});
			subscriber.next(false);
			return disposable.dispose;
		...
		activated$: new rxjs.Observable((subscriber) => {
			const disposable = commandService.onCommandExecuted((c) => {
				...
				if (id === _univerjs_docs.SetTextSelectionsOperation.id || id === AlignOperationCommand.id) {
					...
					subscriber.next(alignType === _univerjs_core.HorizontalAlign.JUSTIFIED);
				}
			});
			subscriber.next(false);
			return disposable.dispose;
		...
		activated$: new rxjs.Observable((subscriber) => {
			...
			const subscription = commandService.onCommandExecuted((c) => {
				if (c.id === _univerjs_docs.RichTextEditingMutation.id) {
					...
					const instance = univerInstanceService.getCurrentUnitForType(_univerjs_core.UniverInstanceType.UNIVER_DOC);
...
function getFontStyleAtCursor(accessor) {
	var _docRanges$find2, _docMenuStyleService$, _paragraph$paragraphS7, _paragraph$paragraphS8;
	const univerInstanceService = accessor.get(_univerjs_core.IUniverInstanceService);
	...
	const paragraph = getParagraphStyleAtCursor(accessor);
	const namedStyle = (paragraph === null || paragraph === void 0 || (_paragraph$paragraphS7 = paragraph.paragraphStyle) === null || _paragraph$paragraphS7 === void 0 ? void 0 : _paragraph$paragraphS7.namedStyleType) ? _univerjs_core.NAMED_STYLE_MAP[paragraph === null || paragraph === void 0 || (_paragraph$paragraphS8 = paragraph.paragraphStyle) === null || _paragraph$paragraphS8 === void 0 ? void 0 : _paragraph$paragraphS8.namedStyleType] : null;
...
let DocEventManagerService = class DocEventManagerService extends _univerjs_core.Disposable {
	...
	_buildBulletBoundsBySegment(segmentId, segmentPage = -1) {
		...
		const paragraphs = ((_body$paragraphs = body === null || body === void 0 ? void 0 : body.paragraphs) !== null && _body$paragraphs !== void 0 ? _body$paragraphs : []).filter((p) => p.bullet && p.bullet.listType.indexOf("CHECK_LIST") === 0);
		const bounds = [];
	...
	_buildParagraphBounds() {
		...
		if (!this._paragraphDirty) return;
	...
	findParagraphBoundByIndex(index) {
		...
		return Array.from(this._tableParagraphBounds.values()).flat().find((bound) => bound.startIndex === index);
	}
};
...
function FloatToolbar(props) {
	...
	(0, react.useEffect)(() => {
		function getRibbon() {
			const flatMenus = menuManagerService.getFlatMenuByPositionKey(_univerjs_ui.MenuManagerPosition.RIBBON);
			const menus = [];
...
let DocParagraphMenuService = class DocParagraphMenuService extends _univerjs_core.Disposable {
	...
	_init() {
		const handleHoverParagraph = (paragraph) => {
			if (this._docSkeletonManagerService.getViewModel().getEditArea() === _univerjs_engine_render.DocumentEditArea.BODY && !this._floatMenuService.floatMenu && !this._context.unit.getDisabled()) {
				var _this$_paragrahMenu2;
				if ((_this$_paragrahMenu2 = this._paragrahMenu) === null || _this$_paragrahMenu2 === void 0 ? void 0 : _this$_paragrahMenu2.active) return;
				if (paragraph) {
		...
		this.disposeWithMe((0, rxjs.combineLatest)([this._docEventManagerService.hoverParagraphRealTime$, this._docEventManagerService.hoverParagraphLeft$]).pipe((0, rxjs.throttleTime)(16)).subscribe(([p, left]) => {
	...
	showParagraphMenu(paragraph) {
		var _this$_paragrahMenu3, _this$_context$unit$g, _this$_context$unit$g2;
		if (((_this$_paragrahMenu3 = this._paragrahMenu) === null || _this$_paragrahMenu3 === void 0 ? void 0 : _this$_paragrahMenu3.paragraph.startIndex) === paragraph.startIndex) return;
		this.hideParagraphMenu(true);
...
const ParagraphMenu = ({ popup }) => {
	var _ref, _paragraphObj$paragra;
	...
	const startIndex = (_ref = paragraph !== null && paragraph !== void 0 ? paragraph : paragraphLeft) === null || _ref === void 0 ? void 0 : _ref.startIndex;
	const paragraphObj = (0, react.useMemo)(() => {
		var _doc$getBody;
		return doc === null || doc === void 0 || (_doc$getBody = doc.getBody()) === null || _doc$getBody === void 0 || (_doc$getBody = _doc$getBody.paragraphs) === null || _doc$getBody === void 0 ? void 0 : _doc$getBody.find((p) => p.startIndex === startIndex);
	}, [doc, paragraph]);
	...
	const handleHideMenu = () => {
		setVisible(false);
		docParagraphMenuService === null || docParagraphMenuService === void 0 || docParagraphMenuService.hideParagraphMenu(true);
	};
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		"data-u-comp": "paragraph-menu",
		ref: anchorRef,
		className: (0, _univerjs_design.clsx)("univer-mr-1 univer-inline-flex univer-h-7 univer-cursor-pointer univer-items-center univer-gap-1 univer-rounded-full univer-px-2.5 univer-py-0 hover:univer-bg-gray-100 dark:hover:!univer-bg-gray-600", _univerjs_design.borderClassName, {
			"univer-bg-gray-100 dark:!univer-bg-gray-700": visible,
			"univer-bg-white dark:!univer-bg-gray-700": !visible
		}),
		onMouseEnter: (e) => {
			var _popup$onPointerEnter, _anchorRef$current, _boundingRect$left, _boundingRect$right, _boundingRect$top, _boundingRect$bottom;
			...
			anchorRect$.next({
				left: ((_boundingRect$left = boundingRect === null || boundingRect === void 0 ? void 0 : boundingRect.left) !== null && _boundingRect$left !== void 0 ? _boundingRect$left : 0) - 4,
		...
		onClick: () => {
			setVisible(true);
			docParagraphMenuService === null || docParagraphMenuService === void 0 || docParagraphMenuService.setParagraphMenuActive(true);
		},
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(icon.component, { className: "univer-size-4 univer-text-gray-900 dark:!univer-text-white" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_univerjs_icons.DownIcon, { className: "univer-size-3 univer-text-gray-600 dark:!univer-text-gray-200" })]
		...
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("section", {
			...
			children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_univerjs_ui.ContextMenuPanel, {
				...
				menuType: _univerjs_ui.ContextMenuPosition.PARAGRAPH,
...
const useCurrentParagraph = () => {
	var _segment$getBody$para, _segment$getBody, _segment$getBody$data, _segment$getBody2, _BuildTextUtils$range;
	const docDataModel = (0, _univerjs_ui.useDependency)(_univerjs_core.IUniverInstanceService).getCurrentUnitForType(_univerjs_core.UniverInstanceType.UNIVER_DOC);
...
const useFirstParagraphIndentSpaceAbove = (paragraph) => {
	...
	const [spaceAbove, setSpaceAboveInternal] = (0, react.useState)(() => {
		...
		const firstParagraph = paragraph[0];
		if (!firstParagraph) return 0;
		return (0, _univerjs_engine_render.getNumberUnitValue)((_firstParagraph$parag7 = firstParagraph.paragraphStyle) === null || _firstParagraph$parag7 === void 0 ? void 0 : _firstParagraph$parag7.spaceAbove, 0);
	});
...
const useFirstParagraphSpaceBelow = (paragraph) => {
	...
	const [spaceBelow, setSpaceBelowInternal] = (0, react.useState)(() => {
		...
		const firstParagraph = paragraph[0];
		if (!firstParagraph) return 0;
		return (0, _univerjs_engine_render.getNumberUnitValue)((_firstParagraph$parag8 = firstParagraph.paragraphStyle) === null || _firstParagraph$parag8 === void 0 ? void 0 : _firstParagraph$parag8.spaceBelow, 0);
	});
...
const useFirstParagraphLineSpacing = (paragraph) => {
	...
	const [lineSpacing, setLineSpacingInternal] = (0, react.useState)(() => {
		...
		const firstParagraph = paragraph[0];
		if (!firstParagraph) return 1;
		return (_firstParagraph$parag9 = (_firstParagraph$parag10 = firstParagraph.paragraphStyle) === null || _firstParagraph$parag10 === void 0 ? void 0 : _firstParagraph$parag10.lineSpacing) !== null && _firstParagraph$parag9 !== void 0 ? _firstParagraph$parag9 : 1;
	});
...
function ParagraphSetting() {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [
		/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
			className: "univer-t-4 univer-text-sm univer-font-medium",
			children: localeService.t("doc.paragraphSetting.alignment")
		}),
		/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
			className: (0, _univerjs_design.clsx)("univer-mt-4 univer-box-border univer-flex univer-w-full univer-items-center univer-justify-between univer-gap-1 univer-rounded-md univer-p-1", _univerjs_design.borderClassName),
		...
		/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "univer-item-center univer-flex univer-justify-between",
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "univer-mt-3 univer-text-xs",
					children: [localeService.t("doc.paragraphSetting.left"), "(px)"]
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "univer-mt-3 univer-text-xs",
					children: [localeService.t("doc.paragraphSetting.right"), "(px)"]
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AutoFocusInputNumber, {
					className: "univer-mt-4",
					value: indentEnd,
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "univer-mt-3 univer-text-xs",
					children: [localeService.t("doc.paragraphSetting.firstLine"), "(px)"]
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AutoFocusInputNumber, {
					className: "univer-mt-4",
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "univer-mt-3 univer-text-xs",
					children: [localeService.t("doc.paragraphSetting.hanging"), "(px)"]
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AutoFocusInputNumber, {
					className: "univer-mt-4",
					value: hanging,
		...
		/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "univer-justify-betweenuniver-item-center univer-flex",
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "univer-mt-3 univer-text-xs",
					children: [localeService.t("doc.paragraphSetting.before"), "(px)"]
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AutoFocusInputNumber, {
					className: "univer-mt-4",
					value: spaceAbove,
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "univer-mt-3 univer-text-xs",
					children: [localeService.t("doc.paragraphSetting.after"), "(px)"]
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AutoFocusInputNumber, {
					className: "univer-mt-4",
					value: spaceBelow,
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "item-center univer-flex univer-justify-between",
...
const menuSchema = {
	...
	[_univerjs_ui.RibbonInsertGroup.MEDIA]: { [TABLE_MENU_ID]: {
		order: 2,
...
function DocFooter() {
	const univerInstanceService = (0, _univerjs_ui.useDependency)(_univerjs_core.IUniverInstanceService);
	if ((0, _univerjs_ui.useObservable)(() => univerInstanceService.getCurrentTypeOfUnit$(_univerjs_core.UniverInstanceType.UNIVER_SHEET), void 0, void 0, [])) return null;
...
let DocRenderController = class DocRenderController extends _univerjs_core.RxDisposable {
	...
	_addNewRender() {
		...
		scene.onMouseWheel$.subscribeEvent((evt, state) => {
			const currentDocUnit = this._univerInstanceService.getCurrentUnitForType(_univerjs_core.UniverInstanceType.UNIVER_DOC);
...
const DocCreateTableConfirm = ({ handleRowColChange, tableCreateParams }) => {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: localeService.t("toolbar.table.rowCount") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_univerjs_design.InputNumber, {
			...
			className: "univer-flex univer-items-center univer-gap-2",
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: localeService.t("toolbar.table.colCount") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_univerjs_design.InputNumber, {
				className: "univer-w-28",
				min: 1,
				max: 20,
				precision: 0,
...
let DocClipboardController = class DocClipboardController extends _univerjs_core.RxDisposable {
	...
	_initLegacyPasteCommand() {
		...
		(_this$_docSelectionRe = this._docSelectionRenderService) === null || _this$_docSelectionRe === void 0 || _this$_docSelectionRe.onPaste$.pipe((0, rxjs.takeUntil)(this.dispose$)).subscribe((config) => {
			var _clipboardEvent$clipb, _clipboardEvent$clipb2, _clipboardEvent$clipb3, _htmlContent;
			...
			const textContent = (_clipboardEvent$clipb2 = clipboardEvent.clipboardData) === null || _clipboardEvent$clipb2 === void 0 ? void 0 : _clipboardEvent$clipb2.getData(_univerjs_ui.PLAIN_TEXT_CLIPBOARD_MIME_TYPE);
			const files = [...((_clipboardEvent$clipb3 = clipboardEvent.clipboardData) === null || _clipboardEvent$clipb3 === void 0 ? void 0 : _clipboardEvent$clipb3.items) || []].filter((item) => _univerjs_ui.imageMimeTypeSet.has(item.type)).map((item) => item.getAsFile()).filter((e) => !!e);
...
let DocContextMenuRenderController = class DocContextMenuRenderController extends _univerjs_core.Disposable {
	...
	_initPointerDown() {
		...
		const documentsSubscription = ((_this$_context = this._context) === null || _this$_context === void 0 || (_this$_context = _this$_context.mainComponent) === null || _this$_context === void 0 ? void 0 : _this$_context.onPointerDown$).subscribeEvent((event) => {
			if (event.button === 2) this._contextMenuService.triggerContextMenu(event, _univerjs_ui.ContextMenuPosition.MAIN_AREA);
		});
...
DocContextMenuRenderController = __decorate([__decorateParam(1, _univerjs_ui.IContextMenuService), __decorateParam(2, _univerjs_core.ICommandService)], DocContextMenuRenderController);
...
let DocSelectionRenderController = class DocSelectionRenderController extends _univerjs_core.Disposable {
	...
	_initialMain(unitId) {
		...
		this.disposeWithMe(document.onPointerDown$.subscribeEvent((evt, state) => {
			if (this._isEditorReadOnly(unitId)) return;
			const docDataModel = this._instanceSrv.getCurrentUnitForType(_univerjs_core.UniverInstanceType.UNIVER_DOC);
...
let DocZoomRenderController = class DocZoomRenderController extends _univerjs_core.Disposable {
	...
	_initRenderRefresher() {
		this._docSkeletonManagerService.currentSkeleton$.subscribe((param) => {
			...
			this.disposeWithMe(scene.onMouseWheel$.subscribeEvent((e) => {
				if (!e.ctrlKey || !this._contextService.getContextValue(_univerjs_core.FOCUSING_DOC)) return;
	...
	_initCommandExecutedListener() {
		...
		this.disposeWithMe(this._commandService.beforeCommandExecuted((command) => {
			if (command.id === SwitchDocModeCommand.id) this._commandService.executeCommand(SetDocZoomRatioCommand.id, {
				zoomRatio: 1,
				unitId: this._context.unitId
			});
		}));
...
let DocsRenderService = class DocsRenderService extends _univerjs_core.RxDisposable {
	...
	_createRenderer(doc) {
		...
		const unitId = doc.getUnitId();
		const workbookId = (_this$_instanceSrv$ge = this._instanceSrv.getCurrentUnitForType(_univerjs_core.UniverInstanceType.UNIVER_DOC)) === null || _this$_instanceSrv$ge === void 0 ? void 0 : _this$_instanceSrv$ge.getUnitId();
...
let UniverDocsUIPlugin = class UniverDocsUIPlugin extends _univerjs_core.Plugin {
	...
	_markDocAsFocused() {
		...
		try {
			const doc = currentService.getCurrentUnitForType(_univerjs_core.UniverInstanceType.UNIVER_DOC);
			if (!doc) return;
